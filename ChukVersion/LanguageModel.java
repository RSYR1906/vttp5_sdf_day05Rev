...
public class LanguageModel {
   ...
   public void buildModel() throws IOException {
      ...
      BufferedReader br = new BufferedReader(reader); // read input text file
      String line;

      while ((line = br.readLine()) != null) { // check if line is empty
         // Clean the line
         line = line.trim().replaceAll("\\p{Punct}", ""); // removing whitespaces and punctuations
         if (line.length() <= 0)
            continue;
         String[] words = line.split("\\s+"); // split the string into string of array with separator as space or
                                              // multiple spaces.

         for (int i = 0; i < words.length - 1; i++) { // looping through the string array to get the current and next
                                                      // word
            String curr = words[i];
   ...
   public void dumpModel() {
      for (String curr : nextWords.keySet()) {        //looping through the keys in the Map
         System.out.printf(">> %s\n", curr);
